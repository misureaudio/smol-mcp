class EMDToolGv2(Tool):
    ...
    def __init__(self):
        ...
        logging.info("EMDToolGv2 initialized. Empirical Mode Decomposition will be computed.")
    ...
    def forward(self, audio_dict, channel=0, max_imfs=None) -> np.ndarray:
        ...
        try:
            ...
            # 1. Unify key lookup
            samples = audio_dict.get('samples', audio_dict.get('audio_samples'))
            metadata = audio_dict.get('metadata', {})

            # 2. Extract Sample Rate
            fs = metadata.get('sample_rate', metadata.get('Fs'))
            ...
            # 4. Return as Dict to preserve metadata
            return {
                'imf_matrix': imfs,
                'sample_rate': fs,  # RETURN THE RATE
                'samples': sig
            }
